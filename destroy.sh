...
echo ""
echo "Note: Local SSH key at ~/.ssh/ssm-key.pem will NOT be deleted"
echo ""
...
echo "  - SSH key: ~/.ssh/ssm-key.pem (delete manually if not needed)"
echo "  - Terraform state: terraform.tfstate"
echo ""
echo "To completely clean up:"
echo "  - Delete SSH key: rm ~/.ssh/ssm-key.pem"
